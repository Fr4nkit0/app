...
void main() {
  ...
  group('recordPayment', () {
    ...
    test(
      'decreases balance on subsequent payments for same customer',
      () async {
    ...
    test('updates lastEntryId on customer balance after payment', () async {
      ...
      final entryRow = await db
          .select(db.customerAccountEntryTable)
          .getSingle();
    ...
    test(
      'rolls back all writes when payment insert fails (duplicate PK)',
      () async {
  ...
  group('watchCustomerBalance', () {
    test('emits 0.0 when no balance row exists for the customer', () async {
      final balance = await sut.watchCustomerBalance('mock-customer-1').first;
      expect(balance, 0.0);
    });
    ...
    test('emits updated balance after a payment is recorded', () async {
      ...
      final sub = sut
          .watchCustomerBalance('mock-customer-1')
  ...
  group('getVisitPayments', () {
    test('returns only payments for the given visitId', () async {
      await sut.recordPayment(makePayment(id: 'p-v1', visitId: 'visit-abc'));
      await sut.recordPayment(makePayment(id: 'p-v2', visitId: 'visit-xyz'));
    ...
    test('returns multiple payments for the same visit', () async {
      await sut.recordPayment(makePayment(id: 'p-a', visitId: 'visit-1'));
      await sut.recordPayment(makePayment(id: 'p-b', visitId: 'visit-1'));
  ...
  group('getCustomerLedger', () {
    ...
    test(
      'returns entries sorted by createdAt descending (most recent first)',
      () async {
