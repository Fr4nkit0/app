...
void main() {
  group('Database Schema Migration V2 -> V3', () {
    ...
    test('Migration creates new tables and preserves customer data', () async {
      ...
      await db.into(db.customerAccountEntryTable).insert(
      ...
      await db.into(db.customerBalanceTable).insert(
      ...
      await db.into(db.customerContainerBalanceTable).insert(
      ...
      final containerBalances = await db.select(db.customerContainerBalanceTable).get();
      expect(containerBalances.length, 1);

      // RouteInventoryLoadTable
      await db.into(db.routeInventoryLoadTable).insert(
